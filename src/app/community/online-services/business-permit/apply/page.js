// src/app/community/online-services/business-permit/apply/page.js"use client";

'use client';

import React from "react";
import { useSelector } from "react-redux";
import PermitForm from "@/components/forms/permitForm";
import AuthLoader from "@/components/common/authLoader";
import { useAuthRedirect } from "@/utils/auth/useAuthRedirect";

export default function BusinessPermitApplyPage() {
  // Use the custom hook to handle auth state and redirection logic
  const { isLoading, isAuthenticated } = useAuthRedirect(
    "/community/online-services/business-permit/apply/"
  );

  // Get user data from Redux, as the hook doesn't handle this
  const { user } = useSelector((state) => state.auth);

  // Show a loading spinner while the auth check is in progress
  if (isLoading) {
    return <AuthLoader message="Verifying Access" type="loading" />;
  }

  // If not authenticated, show a redirecting message. The hook will handle the actual redirect.
  if (!isAuthenticated) {
    return (
      <AuthLoader message="Authentication Required" type="unauthenticated" />
    );
  }

  // If authenticated, render the main page content
  return (
    <main className="min-h-screen py-10 px-4 mt-7 sm:px-8 bg-gray-50 dark:bg-gray-900 font-roboto">
      <div className="max-w-7xl mx-auto space-y-8 lg:space-y-0 lg:flex lg:gap-8">
        
        {/* Left Side: Page Title and User Welcome Info */}
        <div className="lg:w-1/3 space-y-6">
          
          {/* Page Title & Description */}
          <div className="text-left mb-6">
            <h1 className="text-3xl md:text-4xl font-bold font-montserrat text-gray-900 dark:text-gray-100 mb-2">
              Apply for Business Permit
            </h1>
            <p className="text-lg text-gray-600 dark:text-gray-400 mb-2">
              Complete your application below.
            </p>
            <div className="w-24 h-1 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full"></div>
          </div>
          
          {/* Welcome/User Info Card - Redesigned */}
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border-l-4 border-blue-500 dark:border-cyan-500 transition-all duration-300 transform hover:scale-105">
            <div className="flex items-center space-x-4">
              <div className="w-14 h-14 bg-gradient-to-br from-green-500 to-emerald-600 rounded-full flex items-center justify-center flex-shrink-0">
                <svg
                  className="w-8 h-8 text-white"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
                  />
                </svg>
              </div>
              <div className="flex-grow">
                <h2 className="text-xl font-semibold font-montserrat text-gray-900 dark:text-gray-100">
                  Welcome, {user?.full_name || user?.email || "User"}!
                </h2>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  You&apos;re signed in and ready to get started.
                </p>
              </div>
            </div>
          </div>

          {/* Help Section - Redesigned to fit the new panel style */}
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-2xl border-l-4 border-blue-500 dark:border-blue-800 p-6 shadow-md transition-all duration-300 transform hover:scale-105">
            <div className="flex items-start space-x-3">
              <div className="flex-shrink-0">
                <svg
                  className="w-6 h-6 text-blue-600 dark:text-blue-400 mt-0.5"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
                  />
                </svg>
              </div>
              <div>
                <h4 className="font-medium font-montserrat text-blue-900 dark:text-blue-100 mb-1">
                  Need Help?
                </h4>
                <p className="text-sm text-blue-800 dark:text-blue-200 mb-4">
                  If you have questions about your application, our support team is here to help.
                </p>
                <div className="flex flex-col sm:flex-row gap-2">
                  <a
                    href="#"
                    className="inline-flex items-center text-sm font-medium text-blue-700 dark:text-blue-300 hover:text-blue-800 dark:hover:text-blue-200 transition-colors"
                  >
                    <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                    Call Support
                  </a>
                  <a
                    href="#"
                    className="inline-flex items-center text-sm font-medium text-blue-700 dark:text-blue-300 hover:text-blue-800 dark:hover:text-blue-200 transition-colors"
                  >
                    <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
                    Live Chat
                  </a>
                </div>
              </div>
            </div>
          </div>

        </div>

        {/* Right Side: Main Application Form */}
        <div className="lg:w-2/3">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div className="bg-gradient-to-r from-blue-600 to-cyan-600 px-6 py-4">
              <h3 className="text-xl font-semibold font-montserrat text-white">
                Business Permit Application
              </h3>
              <p className="text-blue-100 text-sm mt-1 font-roboto">
                Please fill out all required fields accurately.
              </p>
            </div>
            <div className="p-6">
              <PermitForm />
            </div>
          </div>
        </div>
      </div>
    </main>
  );
}
